...
def test_regular_user_cannot_update_school(db, regular_user_client):
    ...
    response = regular_user_client.put(
        f"/api/v1/schools/{school.id}", json={"name": "Updated Name"}
    )
...
def test_regular_user_can_make_payment(db, regular_user_client):
    ...
    invoice = InvoiceService.create(
        InvoiceCreate(
            ...
            due_date="2024-12-31",
            items=[
                InvoiceItemCreate(description="Tuition", quantity=1, unit_price=1000.00)
            ],
        ),
        db,
    )
    ...
    response = regular_user_client.post(
        ...
        json={"amount": 500.00, "payment_method": "card", "payment_date": "2024-01-15"},
    )
