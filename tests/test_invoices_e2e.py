...
def test_update_invoice_endpoint(client):
    ...
    response = client.put(
        f"/api/v1/invoices/{invoice_id}", json={"due_date": "2024-03-15"}
    )
...
def test_add_item_to_invoice_endpoint(client):
    ...
    response = client.post(
        ...
        json={"description": "New Item", "quantity": 2, "unit_price": "50.00"},
    )
...
def test_update_item_in_invoice_endpoint(client):
    ...
    response = client.patch(
        ...
        json={"description": "Updated Item", "quantity": 3, "unit_price": "200.00"},
    )
...
def test_cannot_delete_last_item_endpoint(client):
    ...
    create_response = client.post(
        ...
        json={
            ...
            "due_date": "2024-02-20",
            "items": [
                {"description": "Only Item", "quantity": 1, "unit_price": "100.00"}
            ],
        },
    )
    invoice = create_response.json()
...
def test_update_nonexistent_invoice(client):
    ...
    response = client.put("/api/v1/invoices/999", json={"due_date": "2024-03-15"})
...
def test_add_item_to_nonexistent_invoice(client):
    ...
    response = client.post(
        "/api/v1/invoices/999/items",
        json={"description": "Item", "quantity": 1, "unit_price": "100.00"},
    )
...
def test_update_nonexistent_item(client):
    ...
    response = client.patch(
        f"/api/v1/invoices/{invoice_id}/items/999",
        json={"description": "Updated", "quantity": 1, "unit_price": "200.00"},
    )
